...
class PoolsideV1ReasoningParser(DeepSeekV3ReasoningParser):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizerBase, *args, **kwargs):
        # PATCH 2026-07-28 (local): default to thinking mode, mirroring
        # DeepSeekV3ReasoningWithThinkingParser. Laguna's chat template defaults
        # enable_thinking=true (generation_config default_chat_template_kwargs),
        # so the model emits <think>...</think> on every request -- but the base
        # DeepSeekV3 parser defaults thinking=False and silently degrades to
        # IdentityReasoningParser, dumping reasoning into content and leaking
        # </think>. Align the parser default with the template default; an
        # explicit thinking/enable_thinking=false in the request still disables.
        chat_kwargs = kwargs.get("chat_template_kwargs", {}) or {}
        if (
            chat_kwargs.get("thinking") is None
            and chat_kwargs.get("enable_thinking") is None
        ):
